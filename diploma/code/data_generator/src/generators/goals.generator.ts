...
export const generateGoals = (count: number): Goal[] => {
  return new Array(count).fill(null).map((): Goal => {
    const createdAt = faker.datatype.datetime({
      min: +moment().startOf("year").subtract(1, "year").toDate(),
      max: +moment().subtract(4, "month").toDate(),
    });
    let deadline;

    if (faker.datatype.boolean()) {
      deadline = faker.datatype.datetime({
        min: +moment().add(2, "months").toDate(),
        max: +moment().add(4, "months").toDate(),
      });
    } else {
      deadline = faker.datatype.datetime({
        min: +moment(createdAt).add(1, "week").toDate(),
        max: +moment(createdAt).add(3, "months").toDate(),
      });
    }
    ...
    if (+deadline > +new Date()) {
      ...
      status = faker.helpers.arrayElement(
        Object.values([GoalStatus.COMPLETED, GoalStatus.CANCELLED])
      );
    }
    ...
    return {
      ...
      notes: generateNotes(
        ...
        Math.min(+new Date(), +moment(createdAt).add("1", "hour").toDate()),
        +moment(deadline).toDate()
      ),
